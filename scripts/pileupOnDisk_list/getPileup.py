...
class GetPileup:
    ...
    def __query_DAS(self):
        ...
        if os.path.exists(f"pileups/{pileup_txt}"):
            os.system(f"rm pileups/{pileup_txt}")
        os.system(f"touch pileups/{pileup_txt}")
        for site in sites:
            Logger.WARNING(f"checking {pileup_dataset} in {site}")
            #print (f"dasgoclient -query='file dataset={pileup_dataset} site={site}'")
            os.system(f"dasgoclient -query='file dataset={pileup_dataset} site={site}' >> pileups/{pileup_txt}")
            with open(f"pileups/{pileup_txt}", "r") as rf:
                if "failure" in rf.read():
                    Logger.WARNING(f"skipping {site} due to {rf.read()}")
                    continue
            os.system(f"sort pileups/{pileup_txt} > pileups/__{pileup_txt}")
            os.system(f"uniq pileups/__{pileup_txt} > pileups/{pileup_txt}")
            os.system(f"rm pileups/__{pileup_txt}")
        with open(f"pileups/{pileup_txt}") as rf:
            ...
            if nfiles == 0:
                ...
                os.system(f"dasgoclient -query='file dataset={pileup_dataset}' > pileups/{pileup_txt}")
