...
cmnd = """
    cmsDriver.py Configuration/GenProduction/python/{name}_cfi.py --python_filename 2023_GEN_postBPix/gen_{name}_cfg.py --eventcontent RAWSIM --customise Configuration/DataProcessing/Utils.addMonitoring --datatier GEN --fileout file:gen_{name}.root --conditions 130X_mcRun3_2023_realistic_postBPix_v6  --beamspot Realistic25ns13p6TeVEarly2023Collision  --step GEN --geometry DB:Extended --era Run3 --no_exec --mc -n -1
"""
...
config.General.workArea = '2023_GEN_postBPix/{name}'
config.General.transferOutputs = True
config.General.transferLogs = True

config.JobType.pluginName = 'PrivateMC'
config.JobType.psetName = '2023_GEN_postBPix/gen_{name}_cfg.py'
...
for f in files:
    name = f.split(".")[0]
    if os.path.exists(f"2023_GEN_postBPix/{name}/crab_{name}"):
        continue
    ...
    with open("2023_GEN_postBPix/crab_submit_%s.py" % name, "w+") as f:
        f.write(crab.format(name=name))
    os.system("crab submit 2023_GEN_postBPix/crab_submit_%s.py" % name)
