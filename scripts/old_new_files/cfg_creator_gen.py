...
config.General.workArea = '2023_GEN-ext/{name}'
config.General.transferOutputs = True
config.General.transferLogs = True

config.JobType.pluginName = 'PrivateMC'
config.JobType.psetName = '2023_GEN-ext/gen_{name}_cfg.py'
...
for f in files:
    name = f.split(".")[0]
    if os.path.exists(f"2023_GEN-ext/{name}/crab_{name}"):
        continue
